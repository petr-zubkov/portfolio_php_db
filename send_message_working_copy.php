<?php
// Рабочий обработчик - копия успешного теста SMTP
...
try {
    ...
    // Используем тот же код, что и в рабочем тесте
    ...
    if ($smtp_sent) {
        ...
        echo json_encode([
            ...
            'debug' => [
                'method' => 'Working SMTP (копия теста)',
                'db_saved' => $db_saved,
                'smtp_sent' => true,
                'to_email' => SMTP_TO_EMAIL,
                'note' => 'Используется тот же код, что и в успешном тесте'
            ]
        ]);
    } else {
...
// Функция отправки SMTP (копия рабочего кода из теста)
